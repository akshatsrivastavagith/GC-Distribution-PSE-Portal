...
# Invocation: python3 voucher_upload_controlled.py <username> <password> <raw_csv_path> <run_folder> <rzp_commission_paisa>

if len(sys.argv) < 6:
    print('\nUsage: python3 voucher_upload_controlled.py username password raw_csv run_folder rzp_commission_paisa')
    sys.exit(1)

USERNAME = sys.argv[1]
PASSWORD = sys.argv[2]
RAW_CSV_PATH = sys.argv[3]
RUN_FOLDER = sys.argv[4]
RZP_COMMISSION = int(sys.argv[5])

API_BASE_URL = "https://offers-engine-live-statuscake.razorpay.com/v1"
API_ENDPOINT = "/offers/voucher-benefits"

# For test detection: if RUN_FOLDER/meta.json env=TEST we'll switch base url
try:
    meta = json.loads(open(os.path.join(RUN_FOLDER,'meta.json')).read())
    if meta.get('env','PROD').upper() == 'TEST':
        API_BASE_URL = "https://offers-engine-test-statuscake.razorpay.com/v1"
except Exception:
    pass
